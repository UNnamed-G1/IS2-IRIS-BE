...
class UserResearchGroup < ApplicationRecord
  ...
  enum member_type: [:miembro, :lider]
  ...
  validates :member_type, inclusion: {in: type_urgs, message: "El tipo de usuario seleccionado no es valido."}

  scope :lider, ->{ where(member_type: :lider) }
