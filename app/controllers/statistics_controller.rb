class StatisticsController < ApplicationController
  def num_publications_by_user
    ...
    if user_id.to_i > num_users
      render json: {
               num_publications_by_user: "El usuario solicitado no se encuentra registrado en nuestra base de datos",
             }, status: :ok
    else
  ...
  def num_publications_by_rg
    rg_id = params[:id]
    num_research_groups = ResearchGroup.all.count
    if rg_id.to_i > num_research_groups
      render json: {
               num_publications_by_rg: "El grupo solicitado no se encuentra registrado en nuestra base de datos",
             }, status: :ok
    else
  ...
  def recent_publications_by_user
    ...
    if user_id.to_i > num_users
      render json: {
               recent_publications_by_user: "El usuario no se encuentra registrado en nuestra base de datos",
             }, status: :ok
    else
  ...
  def recent_publications_by_rg
    ...
    if rg_id.to_i > num_research_groups
      render json: {
               recent_publications_by_rg: "El grupo solicitado no se encuentra registrado en nuestra base de datos",
             }, status: :ok
    else
  ...
  def num_publications_by_user_and_type
    ...
    if user_id.to_i > num_users
      render json: {
               message: "El usuario no se encuentra registrado en nuestra base de datos",
             }, status: :error
    else
      data = Hash.new
      ...
      publication_types = Publication.type_pubs
      if data["publications"].empty?
        ...
        for publication_type in publication_types
          data["stats_publication"][publication_type[0].capitalize] = Publication.num_publications_by_user_and_type(user_id, publication_type[1])
        end
  ...
  def num_publications_by_rg_and_type
    ...
    if rg_id.to_i > num_research_groups
      render json: {
               message: "El grupo solicitado no se encuentra registrado en nuestra base de datos",
             }, status: :error
    else
      data = Hash.new
      ...
      if data["publications"].empty?
        ...
        for publication_type in publication_types
          data["stats_publication"][publication_type[0].capitalize] = Publication.num_publications_by_rg_and_type(rg_id, publication_type[1])
        end
  ...
  def average_publications_in_a_period_by_rg #Monthly average in the last 3 or 6 months
    ...
    if rg_id.to_i > num_research_groups
      render json: {
               num_publications_in_a_period_by_rg: "El grupo solicitado no se encuentra registrado en nuestra base de datos",
             }, status: :ok
    else
      data = Hash.new
      data["research_group_name"] = ResearchGroup.find_by_id(rg_id).name
  ...
  def average_publications_in_a_period_by_user #Monthly average in the last 3 or 6 months
    ...
    if user_id.to_i > num_users
      render json: {
               num_publications_in_a_period_by_user: "El usuario solicitado no se encuentra registrado en nuestra base de datos",
             }, status: :ok
    else
      data = Hash.new
